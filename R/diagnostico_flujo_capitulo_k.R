...
diagnostico_flujo_capitulo_k <- function(dfs,
  ...
  data_a <- if ("A" %in% names(dfs) && is.data.frame(dfs$A)) {
    tibble::as_tibble(dfs$A) %>%
      .diag_k_normalizar_utf8_df()
  } else {
    NULL
  }
  K_eval <- .diag_k_agregar_edad(K, data_edad, llaves_persona)
  K_eval <- .diag_k_agregar_territorio_a(K_eval, data_a)
...
.diag_k_extraer_variables_regla <- function(regla_r) {
  ...
  vars <- unique(unlist(regmatches(
    regla_r,
    gregexpr("NPCK[A-Z0-9_]+|llega_K[0-9]+|edad|NPCKP17_FINAL|codigo_municipio|Clase|aplica_npckp45l_caballo", regla_r)
  )))
...
.diag_k_agregar_territorio_a <- function(K, data_a) {
  if (is.null(data_a) || !is.data.frame(data_a) || !"DIRECTORIO" %in% names(data_a)) {
    if (!"codigo_municipio" %in% names(K)) K$codigo_municipio <- NA_character_
    if (!"Clase" %in% names(K)) K$Clase <- NA_character_
    return(K)
  }

  var_mpio <- col_first_existing(data_a, c("codigo_municipio", "COD_MPIO", "Mpio", "MPIO", "Municipio", "Munic", "MUNICIPIO"))
  var_clase <- col_first_existing(data_a, c("Clase", "CLASE", "clase"))
  if (is.null(var_mpio) && is.null(var_clase)) {
    if (!"codigo_municipio" %in% names(K)) K$codigo_municipio <- NA_character_
    if (!"Clase" %in% names(K)) K$Clase <- NA_character_
    return(K)
  }

  cols_a <- c("DIRECTORIO", stats::na.omit(c(var_mpio, var_clase)))
  territorio_a <- data_a %>%
    normalize_keys("DIRECTORIO") %>%
    dplyr::select(dplyr::all_of(cols_a)) %>%
    dplyr::distinct(.data$DIRECTORIO, .keep_all = TRUE) %>%
    dplyr::transmute(
      DIRECTORIO = .data$DIRECTORIO,
      codigo_municipio = if (!is.null(var_mpio)) as.character(.data[[var_mpio]]) else NA_character_,
      Clase = if (!is.null(var_clase)) as.character(.data[[var_clase]]) else NA_character_
    )

  K %>%
    dplyr::select(-dplyr::any_of(c("codigo_municipio", "Clase"))) %>%
    dplyr::left_join(territorio_a, by = "DIRECTORIO")
}
...
.diag_k_construir_reglas <- function(data,
  ...
  debe_npckp17 <- .diag_k_and(
    universo_k,
    .diag_k_or(eq("NPCKP18", 2), eq("NPCKP19", 1), in_set("NPCKP20", c(1, 2, 9)))
  )
  regla_npckp17 <- paste0(
    "edad >= 10 & (",
    "NPCKP18 == 2 | NPCKP19 == 1 | NPCKP20 %in% c(1,2,9)",
    ")"
  )
  ocupado_k23 <- .diag_k_and(universo_k, in_set(variable_k23_final, 1:8))
  var_ocupado_final <- if ("NPCKP17_FINAL" %in% names(data)) "NPCKP17_FINAL" else variable_k23_final
  ocupado_final <- .diag_k_and(universo_k, in_set(var_ocupado_final, 1:8))
  regla_ocupado_final <- paste0(var_ocupado_final, " %in% c(1,2,3,4,5,6,7,8)")
  ...
  no_ocupado <- .diag_k_and(
    universo_k,
    .diag_k_or(eq("NPCKP1", 5), eq("NPCKP7", 2), eq("NPCKP13", 2))
  )
  ...
  aplica_npckp45l_caballo <- .diag_k_or(ne_val("codigo_municipio", 11001), eq("Clase", 3))
  regla_npckp45l_caballo <- paste0(
    regla_ocupado_final,
    " & NPCKP44A != 1 & (codigo_municipio != 11001 | Clase == 3)"
  )
  comentario_npckp45l_caballo <- paste(
    "NPCKP45L corresponde a Caballo. Segun formulario, aplica solo para",
    "Bogota rural y municipios. Bogota se identifica con codigo_municipio",
    "== 11001 y rural con Clase == 3."
  )
  ...
  add("NPCKP18", "03_ocupados_pre_k23", debe_npckp18, "((!is.na(NPCKP14) & NPCKP14 != \"\") | (!is.na(NPCKP15) & NPCKP15 != \"\") | (!is.na(NPCKP16) & NPCKP16 != \"\"))")
  ...
  add_many(c(
    "NPCKP38B", "NPCKP39", "NPCKP41", "NPCKP43", "NPCKP44",
    "NPCKP44A", "NPCKP47", "NPCKNP48"
  ), "07_bloque_comun_ocupados", ocupado_k23, paste0(variable_k23_final, " %in% c(1,2,3,4,5,6,7,8)"))
  add("NPCKP38A", "07_bloque_comun_ocupados", .diag_k_and(universo_k, eq(variable_k23_final, 6)), paste0(variable_k23_final, " == 6"))
  add_many(c(
  ...
  add(
    ...
    .diag_k_and(ocupado_final, .diag_k_not(eq("NPCKP44A", 1)), aplica_npckp45l_caballo),
    regla_npckp45l_caballo,
    universo_base = "Personas ocupadas finales con desplazamiento al trabajo",
    comentario = comentario_npckp45l_caballo
  )
  ...
  add("NPCKP47C", "08_no_ocupados_posterior", no_ocupado, "NPCKP1 == 5 | NPCKP7 == 2 | NPCKP13 == 2")
  comentario_k60 <- "K60 se responde solo cuando en K59 la persona informa que no ha trabajado alguna vez por al menos dos semanas consecutivas; NPCKP47C == 2."
  add("NPCKP60_1", "08_no_ocupados_posterior", .diag_k_and(universo_k, eq("NPCKP47C", 2)), "NPCKP47C == 2", comentario = comentario_k60)
  add("NPCKP60_2", "08_no_ocupados_posterior", .diag_k_and(universo_k, eq("NPCKP47C", 2)), "NPCKP47C == 2", comentario = comentario_k60)
  ...
  add("NPCKP48", "08_no_ocupados_posterior", no_ocupado, "NPCKP1 == 5 | NPCKP7 == 2 | NPCKP13 == 2")
  add("NPCKP48A", "08_no_ocupados_posterior", eq("NPCKP48", 1), "NPCKP48 == 1")

  regla_pensiones_ingresos <- paste0("edad >= 10 & (NPCKP1 == 5 | NPCKP7 == 2 | NPCKP13 == 2 | NPCKP47B tiene valor | ", variable_k23_final, " %in% c(1,2,3,4,5,6,7,8))")
  debe_npckp50 <- .diag_k_and(
    gt_val(edad, 14),
    .diag_k_or(ocupado_k23, eq("NPCKP1", 5), eq("NPCKP7", 2), in_set("NPCKP13", c(1, 2)), tiene_valor("NPCKP47B"))
  )
  regla_npckp50 <- paste0("edad >= 15 & (NPCKP1 == 5 | NPCKP7 == 2 | NPCKP13 %in% c(1,2) | NPCKP47B tiene valor | ", variable_k23_final, " %in% c(1,2,3,4,5,6,7,8))")
  comentario_npckp50 <- "Se incorpora restriccion explicita del formulario: K64/NPCKP50 solo se formula a personas de 15 anos y mas."
  add_many(c("NPCKP50_A", "NPCKP50_B"), "09_pensiones_ingresos", ocupados_o_no_ocupados, regla_pensiones_ingresos)
  add("NPCKP50", "09_pensiones_ingresos", debe_npckp50, regla_npckp50, comentario = comentario_npckp50)
  add(
    "NPCKP51",
    "09_pensiones_ingresos",
    .diag_k_and(gt_val(edad, 14), eq("NPCKP50", 1)),
    "edad >= 15 & NPCKP50 == 1",
    comentario = "NPCKP51 depende de K64/NPCKP50; por restriccion del formulario aplica solo a personas de 15 anos y mas y cuando NPCKP50 == 1."
  )
  for (par in list(
    ...
    add(par[1], "09_pensiones_ingresos", ocupados_o_no_ocupados, regla_pensiones_ingresos)
  ...
  comentario_k76 <- "Se incorpora restriccion explicita del formulario: K76 solo se formula a personas de 18 anos y mas."
  add("NPCKPN62A", "10_emprendimiento_renta", .diag_k_and(universo_k, gt_val(edad, 17)), "edad >= 18", comentario = comentario_k76)
  add("NPCKPN62B", "10_emprendimiento_renta", .diag_k_and(universo_k, gt_val(edad, 17), eq("NPCKPN62A", 1)), "edad >= 18 & NPCKPN62A == 1", texto_libre = TRUE, comentario = comentario_k76)
  ...
  comentario_k78 <- "Se incorpora corte explicito posterior a K77: si es mayor de 18 anos continua a K78; de lo contrario termina el capitulo."
  regla_k78 <- paste0("edad >= 18 & (NPCKP1 == 5 | NPCKP7 == 2 | NPCKP13 == 2 | NPCKP47B tiene valor | ", variable_k23_final, " %in% c(1,2,3,4,5,6,7,8))")
  add("NPCKP78_1", "12_acoso_laboral", mayor_18_ocupados_o_no_ocupados, regla_k78, comentario = comentario_k78)
  add("NPCKP78_2", "12_acoso_laboral", mayor_18_ocupados_o_no_ocupados, regla_k78, comentario = comentario_k78)
